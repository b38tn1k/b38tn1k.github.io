...
vec4 renderTerrain(vec2 uv) {
  ...
  // Darker, more mineral cave palette
  vec3 darkRock = vec3(0.05, 0.06, 0.08);
  vec3 midRock  = vec3(0.16, 0.17, 0.22);
  vec3 lightRock = vec3(0.38, 0.42, 0.52);

  // Base color from height and lighting (less brightening from lightRock)
  vec3 rockColor = mix(darkRock, midRock, height);
  rockColor = mix(rockColor, lightRock, diffuse * 0.55);

  // Subtle cool water tint toward deeper areas (reduced strength)
  vec3 waterTint = vec3(0.10, 0.20, 0.30);
  float cavityFactor = (1.0 - height) * (0.35 + 0.45 * (1.0 - surf));
  rockColor = mix(rockColor, rockColor * waterTint, cavityFactor * 0.22);

  // Surface grit
  float grit = noise(p * 40.0);
  rockColor += grit * 0.05;

  // Pebble shading (subtle) so large-scale rock patterns stay dominant
  float pebble = noise(p * 25.0) * 0.2 + noise(p * 60.0) * 0.8;
  float pebbleShade = smoothstep(0.55, 0.85, pebble);
  rockColor -= vec3(0.10, 0.10, 0.11) * pebbleShade * 0.18;

  // Moisture streaks on vertical surfaces (keep, but less neon)
  float wet = fbm(p * vec2(1.0, 6.0) + vec2(0.0, -uTime * 0.08));
  wet = smoothstep(0.65, 0.95, wet);
  rockColor += vec3(0.05, 0.08, 0.11) * wet * 0.16;

  // Apply ambient occlusion more strongly to get deeper shadows
  rockColor *= (0.55 + ao * 0.40);

  // ---- Grey Pebbles Layer (darker and softer) ----
  float pebbleN = noise(p * 55.0) * 0.4 + noise(p * 120.0) * 0.6;
  float pebbleMask = smoothstep(0.55, 0.8, pebbleN) * 0.6;
  vec3 greyA = vec3(0.30, 0.32, 0.38);
  vec3 greyB = vec3(0.55, 0.58, 0.68);
  vec3 greyPebble = mix(greyA, greyB, noise(p * 22.0));
  greyPebble *= 0.75 + height * 0.2;
  rockColor = mix(rockColor, greyPebble, pebbleMask);

  // ---- Moss Layer (dimmer, more subtle) ----
  float mossFibers = fbm(p * vec2(6.0, 25.0) + vec2(0.0, 0.2));
  float mossMask = smoothstep(0.45, 0.70, mossFibers);
  mossMask *= (ao * 0.8 + 0.2);
  mossMask *= (wet * 0.9);
  vec3 mossA = vec3(0.06, 0.40, 0.10);
  vec3 mossB = vec3(0.10, 0.50, 0.15);
  vec3 mossColor = mix(mossA, mossB, noise(p * 8.0));
  rockColor = mix(rockColor, mossColor, mossMask * 0.9);

  // ---- Directional Strata Layer with tunnel curvature influence (less contrast)
  float strata = sin(p.y * 20.0 + surf * 3.0 + noise(p * 4.0) * 1.5);
  float strataMask = smoothstep(-0.10, 0.40, strata);
  vec3 strataColor = mix(vec3(0.20, 0.21, 0.26),
                         vec3(0.32, 0.34, 0.40),
                         noise(p * 3.0));
  float strataAmount = strataMask * (0.12 + ao * 0.25);
  rockColor = mix(rockColor, strataColor, strataAmount);

  // Final global darkening to ensure rocks sit under the water in brightness
  rockColor *= 0.90;
