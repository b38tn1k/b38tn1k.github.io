    ...
    display(offAlpha = 0) {
        ...
        if (this.numEdges === 0) {
            ...
            if (this.isAnimating) {
                ...
                if (
                    (this.animationCountDown >= 1 && this.animateForward) ||
                    (this.animationCountDown <= 0 && !this.animateForward)
                ) {
        ...
        if (this.isActive) {
            for (let i = 0; i < this.buttons.length; i++) {
                this.buttons[i].display(
                    map(i, 0, this.buttons.length, 0, TWO_PI) - radians(90)
                );
            }
        }
    }
...
class CircularMenu {
    ...
    updateButtonPositions() {
        for (let i = 0; i < this.buttons.length; i++) {
            ...
            const x =
                this.position.x +
                cos(angle + this.animationCountDown) * this.menuRadius;
            const y =
                this.position.y +
                sin(angle + this.animationCountDown) * this.menuRadius;
        ...
        for (let i = 0; i < this.subMenus.length; i++) {
            for (let j = 0; j < this.subMenus[i].buttons.length; j++) {
                const angle = map(
                    j,
                    0,
                    this.subMenus[i].buttons.length,
                    0,
                    TWO_PI
                );
                const x =
                    this.position.x +
                    cos(angle - this.animationCountDown) * this.menuRadius * 2;
                const y =
                    this.position.y +
                    sin(angle - this.animationCountDown) * this.menuRadius * 2;
    ...
    activateSubMenu(label) {
        ...
        //     this.activeSubMenu = null;
        // } else {
        this.activeSubMenu = this.subMenus.find(
            (subMenu) => subMenu.label === label
        );
        if (this.activeSubMenu) {
