...
function draw(){
  ...
  for (let i = 0; i < myText.length; i+=1){
    let newString = myText[(i + int(offset))%myText.length];
    // if (random() > 0.8){
    //   for (let j = 0; j < noSkip.length; j++){
    //     let n = newString.search(noSkip[j])
    //     if (n >0){
    //       console.log('hi');
    //     }
    //   }
    // }
    text(newString, xoffset, (i + 2)*myTextSize + myTextSize*2)
  }
  if (touchDown == true){
