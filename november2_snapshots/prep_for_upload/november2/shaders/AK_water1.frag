...
vec4 renderWater(vec2 uv) {
  ...
  // Wave bands layered on top (inspired by sea_octave, but softer)
  vec2 waveSpace = (uv - vec2(0.5, 0.7)) * vec2(6.0, 3.5);
  float waves = getWaves2D(waveSpace, uTime * 0.6); // 0..~2-ish
  waves = clamp(waves, 0.0, 2.0);

  // Base field is fbm + waves; Worley used only as a subtle accent
  float combinedBase = t * (0.65 + 0.35 * waves);

  // Extract only the brightest Worley ridges, scaled way down
  float worleyAccent = smoothstep(0.60, 0.95, worleyRaw) * 0.15;

  // Final combined caustic field: mostly smooth, gently modulated by Worley
  float combined = combinedBase + worleyAccent;

  // Aggressively smooth out small bright islands to avoid bubble noise
  combined = smoothstep(0.30, 0.85, combined);
  ...
  worleyColor = mix(worleyColor,
                    vec3(0.02, 0.35, 0.65) * (combined * 0.6 + nebula * 0.8),
                    0.5);
  ...
  // Blend depth gradient with Worley+wave caustics
  vec3 waterColor = mix(depthColor, worleyColor, 0.8);
  ...
  waterColor += shafts * vec3(0.05, 0.18, 0.24) * 0.6;
