...
function segmentation() {
    ...
    arc(aniWidthOn2 + ofs, aniHeightOn2 + ofs, cr, cr, start, start + inc);
    start += inc;
    arc(aniWidthOn2 - ofs, aniHeightOn2 + ofs, cr, cr, start, start + inc);
    start += inc;
    arc(aniWidthOn2 - ofs, aniHeightOn2 - ofs, cr, cr, start, start + inc);
...
function takeout() {
    ...
    arc(aniWidthOn2 + updateValX, aniHeightOn2 - updateValY, cr, cr, 0, HALF_PI);
    fill(rCol[1]);
    arc(aniWidthOn2 + updateValX, aniHeightOn2 - updateValY, cr, cr, PI, TWO_PI);
    fill(rCol[2]);
    arc(aniWidthOn2 - updateValX, aniHeightOn2 + updateValY, cr, cr, HALF_PI, PI);
}
...
function localquality() {
    ...
    if (ltime % 6 == 0) {
        line(gap, cr, gap + pos, cr);
        image(aniLayers["pencil"], gapOn2 + pos, cr - gap3);
    } else if (ltime % 6 == 1) {
        line(gap, cr, igap, cr);
        image(aniLayers["pencil"], igap - gapOn2, cr - gap3);
    } else if (ltime % 6 == 2) {
        line(gap, cr, igap, cr);
        image(aniLayers["ipencil"], igap - gapOn2, cr - gap3);
    } else if (ltime % 6 == 3) {
        line(gap, cr, igap - pos, cr);
        image(aniLayers["ipencil"], igap - pos - gapOn2, cr - gap3);
    } else if (ltime % 6 == 4) {
        image(aniLayers["ipencil"], gapOn2, cr - gap3);
    } else {
        image(aniLayers["pencil"], gapOn2, cr - gap3);
    }
}
...
function asymmetry() {
    ...
    for (let i = 0; i < 17; i++) {
        fill(rCol[i % 2]);
        arc(aniWidthOn2 + wobble, aniHeightOn2, cr, cr, start, start + inc);
        start += inc;
    }
    if (ltime % 2 == 1) {
        fill(255);
        rect(0, cr, width, height);
    }
    fill(rCol[0]);
    circle(aniWidthOn2 + wobble, aniHeightOn2, icr);
}
...
function merging() {
    ...
    stroke(rCol[2]);
    line(gap, inc, gap, height - inc);
    line(l2s, inc, l2s, height - inc);
    line(l2e, icr + inc + inc * sin(millis() / 300), l2e, 2 * inc + inc * sin(millis() / 300) + 3.8 * gap);
    ...
    for (let i = 0; i < 4; i++) {
        ...
        line(gap, icr + i * icr, gap2, flye);
        stroke(rCol[1]);
        let lys = icr + i * icr;
        let lye = lys + inc + inc * sin(millis() / 300);
        line(l2s, lys, l2e, lye);
    }
}
...
function universality() {
    ...
    fill(rCol[1]);
    rect(rx, r1y, gap2, gapOn2);
    rect(rx, r2y, gap2, gapOn2);
    fill(rCol[0]);
    triangle(tri1[0], tri1[1], tri1[2], tri1[3], tri1[4], tri1[5]);
    // stop
    fill(rCol[0]);
...
function matryoshka() {
    ...
    fill(rCol[0]);
    arc(aniWidthOn2, aniHeightOn2, majorDim, 2 * majorDim, PI, TWO_PI);
    arc(aniWidthOn2, aniHeightOn2, majorDim, majorDim, TWO_PI, PI);
    fill(rCol[1]);
    majorDim *= 2;
    mdOn2 *= 2;
    arc(aniWidthOn2, aniHeightOn2 - timer * mdOn2 - mdOn2, majorDim, 2 * majorDim, PI, TWO_PI);
    arc(aniWidthOn2, aniHeightOn2 + timer * mdOn2 + mdOn2, majorDim, majorDim, TWO_PI, PI);
    fill(rCol[2]);
    majorDim *= 2;
    mdOn2 *= 2;
    arc(aniWidthOn2, aniHeightOn2 - timer * mdOn2 - mdOn2, majorDim, 2 * majorDim, PI, TWO_PI);
...
function antiweight() {
    ...
    noStroke();
    fill(0);
    circle(aniWidthOn2, aniHeightOn3, gapOn2);
    circle(axs, ays, gapOn4);
    circle(axe, aye, gapOn4);
    fill(rCol[0]);
    rect(axe - gapOn4, aye + gap + gapOn2 - mStroke, gapOn2, gapOn2);
    fill(rCol[1]);
    let by = ays + gap + gapOn2 - mStroke;
    rect(axs - gapOn2, by, gap, gapOn2);
    strokeWeight(mStroke);
    if (!timer2) {
        fill(255);
        stroke(255);
        rect(axs - gapOn4, by + gapOn4 / 2, gapOn2, gapOn4);
    }
    stroke(0);
    strokeWeight(mStroke);
    noFill();
    line(axs, ays, axe, aye);
    triangle(axs, ays + mStroke, axs - gap, ays + gap2, axs + gap, ays + gap2);
    triangle(axe, aye + mStroke, axe - gap, aye + gap2, axe + gap, aye + gap2);
}
...
function pantiactions() {
    ...
    let cx = width - (gap + gapOn2); //cy = gap
    ...
    stroke(mc);
    strokeWeight(mStroke / 2);
    let toggle = true;
    let cs = 0;
    for (let i = -2; i <= 2; i += 0.5) {
        toggle = true;
        for (let j = gap; j < height - gapOn2; j += gapOn4) {
            if (toggle) {
                line(cx + i * gapOn2, j, cx + i * gapOn2, j + gapOn4 / 2);
                cs++;
            }
            if (cs % 2 == 0) {
                stroke(mc);
    ...
    fill(220); //clouds
    for (let i = -2; i <= 2; i++) {
        circle(cx + i * gapOn2, gap, gapOn2);
    }
    for (let i = -1.5; i <= 1.5; i++) {
        circle(cx + i * gapOn2, gap34, gapOn2);
        circle(cx + i * gapOn2, gapOn4 + gap, gapOn2);
    }
    fill(rCol[0]); //umbrella
    let osc = aniWidthOn2 + sin(millis() / 2000) * aniWidthOn3;
    ellipse(osc, aniHeightOn3 * 2, gap2, gap);
    fill(255);
    rect(osc - gap, aniHeightOn3 * 2, gap2, height);
    for (let i = -1; i <= 1; i++) {
        circle(osc + i * gapOn2, aniHeightOn3 * 2 + gapOn4 / 2, gapOn2);
    }
    stroke(0);
    strokeWeight(mStroke);
    line(osc, aniHeightOn3 * 2 - gapOn4 / 2, osc, height - gapOn2);
    point(osc, aniHeightOn3 * 2 - gapOn2);
    fill(255);
    stroke(rCol[0]);
    arc(osc - gapOn4 / 2, height - gapOn2, gapOn4, gapOn4, 0, PI);
}
...
function aniRulers() {
    stroke(0);
    strokeWeight(1);
    line(0, aniHeightOn2, width, aniHeightOn2);
    line(aniWidthOn2, 0, aniWidthOn2, height);
    // line(0, aniHeightOn2 - (gap + gapOn2), width, aniHeightOn2 - (gap + gapOn2));
    // line(0, aniHeightOn2 + gap, width, aniHeightOn2 + gap);
}
...
function preactions() {
    ...
    //draw layout
    square(warehouse[0], warehouse[1], gap2);
    for (let i = 0; i < sats.length; i++) {
        square(sats[i][0], sats[i][1], gap);
    }
    ...
    for (let i = 0; i < houses.length; i++) {
        ...
        square(houses[i][0], houses[i][1], gapOn2);
    }
    fill(0);
    square(sats[0][0], sats[0][1], gapOn2);
    square(sats[1][0], sats[1][1], gapOn2);
    // transport test
    fill(rCol[1]);
    noStroke();
    let c = deliver(sats[int(ind / 2)], houses[ind], period, ind);
    square(c[0], c[1], gapOn2 - mStroke);
    ...
    square(c[0], c[1], gapOn2 - mStroke);
    square(c[0], c[1] + (gapOn2 + mStroke), gapOn2 - mStroke);
    ...
    square(c[0], c[1], gapOn2 - mStroke);
    rectMode(CORNER);
    fill(255); //square(warehouse[0], warehouse[1], gap2);
    rect(0, 0, warehouse[0] - gap * 1.05, height);
}
...
function beforehandcushioning() {
    ...
    rect(x, ah32, gap2, gap);
    fill(rCol[1]);
    circle(x - gapOn2, ah32 + gapOn2, gap34);
    circle(x + gapOn2, ah32 + gapOn2, gap34);
    fill(rCol[2]);
    rect(width - aniWidthOn4, aniHeightOn2, aniWidthOn3 - gap, aniHeightOn2 + gap);
    stroke(0);
    strokeWeight(mStroke / 2);
    let spx = x + gap + 1;
    ...
    for (let i = 0; i < 5; i++) {
        line(spx, ah32 - gapOn4, spx, ah32 + gapOn4);
        if (i < 4) {
            line(spx, ah32 - gapOn4, spx + inc, ah32 + gapOn4);
        }
        spx += inc;
    }
    rectMode(CORNER);
...
function equipotentiality() {
    ...
    rect(0, low, aniWidthOn3, gap);
    rect(aniWidthOn3 * 2, high, aniWidthOn3, gap + aniHeightOn3);
    rect(aniWidthOn3, low, aniWidthOn3, gap);

    let lockHeight = low - t * high;

    rect(aniWidthOn3, low - t * high, aniWidthOn3, gap + t * high);
    let boatx = 0;
    let boaty = 0;
    ...
    fill(255, 0, 0);
    rectMode(CENTER);
    rect(boatx, boaty, gap, gapOn2);
    triangle(boatx - gapOn2, boaty - gapOn4, boatx - gap, boaty - gapOn4, boatx - gapOn2, boaty + gapOn4);
    triangle(boatx + gapOn2, boaty - gapOn4, boatx + gap, boaty - gapOn4, boatx + gapOn2, boaty + gapOn4);
    rect(boatx, boaty - gap34, gapOn6 / 2, gap);
    fill(255, 255, 0);
    let mast = boatx - gapOn6 / 2;
    triangle(mast, boaty - (gap + gapOn4), mast, boaty - gapOn4, mast - gap34, boaty - gapOn4);
    rectMode(CORNER);
    strokeWeight(mStroke);
    stroke(0);
    line(aniWidthOn3, aniWidthOn3 - gap, aniWidthOn3, l1);
    line(2 * aniWidthOn3, l2, 2 * aniWidthOn3, aniHeightOn3 * 2 + gap);
}
...
function dothingsbackwards() {
    ...
    fill(139, 69, 19);
    rect(aniWidthOn4, ah42, gapOn3, aniWidthOn2);
    ...
    rect(wpx, ah42, gap, gap + gapOn2);
    fill(190);
    rect(aw42, ah42 + gapOn4, gap2, gap);
    triangle(nailx, vy1, nailx, vy2, nailx + gap, vy3);
}
...
function curvature() {
    ...
    rect(cx, tb, aw315, cr / 2);
    rect(cx, cy + cr, aw315, cr / 2);
    fill(255);
    circle(cx, cy, cr);
    fill(rCol[1]);
    circle(mcx, mcy, gapOn2);
    rect(2 * aniWidthOn3 + tx1 * cr, tb + cr / 2, gapOn4, cr * 2);
    strokeWeight(mStroke);
    stroke(rCol[2]);
    line(mcx, mcy, 2 * aniWidthOn3 + tx1 * cr + gapOn4 / 2, tb + cr * 1.5);
}
...
function dynamics() {
    ...
    let cyp = cy - ah8;
    line(cx, cy, cx, cyp);
    line(cx - gap, cyp, cx + gap, cyp);
    let pl = aniHeightOn2;
    let inc = (cy - pl) / 3;
    let pos = inc;
    for (let i = 0; i < 3; i++) {
        line(cx - gap, cyp - pos, cx - gapOn2, cyp - pos);
        pos += inc;
    }
    let bb = aniHeightOn4 + gapOn2;
    line(cx + gap34, cyp, cx + gap34, cyp - gapOn6);
    line(cx + gap34, bb, cx + gap34, bb + gapOn6);
    stroke(220);
    line(0, aniHeightOn4 + gapOn4, width, aniHeightOn4 + gapOn4);
    noStroke();
    fill(rCol[1]);
    rect(cx - gap, aniHeightOn4, gap2, gapOn2);
    fill(rCol[2]);
    rect(cx + gapOn2, bb + gapOn6, gapOn2, cyp - bb - gapOn3);
    image(aniLayers["dynSin"], 0, aniHeightOn2);
    image(aniLayers["dynSin"], aniWidthOn2, aniHeightOn2);
}
...
function partialorexcessive() {
    ...
    background(rCol[prevInd]);
    let xPaint;
    if (stencilTimer >= 0) {
        xPaint = paintTimer * width;
    } else {
        xPaint = width;
    }
    rect(0, 0, xPaint, height);
    ...
    // square( sp, aniHeightOn2 - gapOn2, gap);
}
...
function drawTruck() {
    if (!("truck" in aniLayers)) {
        aniLayers["truck"] = createGraphics(width, height);
        aniLayers["truck"].noStroke();
        aniLayers["truck"].fill(100);
        let aw8 = aniWidthOn4 / 2;
        aniLayers["truck"].rect(0, height - aniHeightOn4 - gapOn2, aniWidthOn4 * 3, gapOn2);
        aniLayers["truck"].arc(aw8 + aniWidthOn2, height - aniHeightOn4, 6 * gapOn4, 6 * gapOn4, PI, 0);
        ...
        aniLayers["truck"].circle(aw8, height - aniHeightOn4, gap);
        aniLayers["truck"].circle(aw8 + aniWidthOn2, height - aniHeightOn4, gap);
        aniLayers["truck"].fill(100);
        aniLayers["truck"].circle(aw8, height - aniHeightOn4, gapOn3);
        aniLayers["truck"].circle(aw8 + aniWidthOn2, height - aniHeightOn4, gapOn3);
    }
}
...
function anotherdimension() {
    ...
    let cx = width - gapOn2;
    let cy = height - aniHeightOn4 + gapOn4;
    fill(rCol[ind]);
    if (osc > 0.6 && fall > 0) {
        circle(cx - abs(fall * gapOn4), cy - abs(fall * gap), gapOn2);
    } else if (fall < 0) {
        circle(cx, cy, gapOn2);
    }
    strokeWeight(mStroke * 2);
    ...
    fill(150);
    push();
    translate(bcx + sin(osc) * aniWidthOn2, bcy - sin(osc) * aniHeightOn3);
    rotate(osc);
    rect(0, 0, aniWidthOn2, aniHeightOn3);
    fill(200);
    for (let i = 1; i < 12; i += 2) {
        rect(gapOn4 * i, gapOn4, gapOn4, aniHeightOn3 - gapOn2);
    }
    pop();
    image(aniLayers["truck"], 0, 0);
    stroke(0);
}
...
function vibrations() {
    ...
    fill(c1);
    rect(aniWidthOn2 + osc, aniHeightOn3 + osc, aniWidthOn3, aniHeightOn3);
    fill(c2);
    rect(aniWidthOn2 + osc, 2 * aniHeightOn3 + osc, aniWidthOn3, aniHeightOn3);
    strokeWeight(mStroke);
    stroke(c1);
    line(aniWidthOn3 + osc, aniHeightOn2 + osc, aniWidthOn3 * 2 + osc, aniHeightOn2 + osc);
    stroke(0);
    noFill();
    rect(aniWidthOn2 + osc, aniHeightOn2 + osc, aniWidthOn3, aniHeightOn3 * 2);
    rectMode(CORNER);
}
...
function periodicaction() {
    ...
    noStroke();
    circle(sx + t2 * (ex - sx), ty, gapOn3);

    let batchT = countTo3(2 * per);
    let diffx = ex - sx - gap;
    if (batchT == 0) {
        circle(gap + sx + t2 * diffx, by, gapOn3);
        circle(gap + gapOn2 + sx + t2 * diffx, by, gapOn3);
        circle(gap2 + sx + t2 * diffx, by, gapOn3);
    } else if (batchT == 1) {
        circle(sx + t2 * gap2, by, gapOn3);
    } else if (batchT == 2) {
        circle(sx + gap2, by, gapOn3);
        circle(sx + t2 * (gap + gapOn2), by, gapOn3);
    } else if (batchT == 3) {
        circle(sx + gap + gapOn2, by, gapOn3);
        circle(sx + gap2, by, gapOn3);
        circle(sx + t2 * gap, by, gapOn3);
    }
    fill(255);
    rect(ex + gap, aniHeightOn2, gap2, height);
    fill(rCol[1]);
    square(sx, ty, gap);
    square(sx, by, gap);
    fill(rCol[0]);
...
function continuityofusefulaction() {
    ...
    let timer = sin(millis() / per);
    push();
    translate(aniWidthOn2, height - aniHeightOn3);
    rotate(TWO_PI * (millis() / per));
    circle(0, 0, gap);
    fill(100);
    arc(0, 0, gap, gap, 0, QUARTER_PI);
    arc(0, 0, gap, gap, HALF_PI, HALF_PI + QUARTER_PI);
    arc(0, 0, gap, gap, PI, PI + QUARTER_PI);
    arc(0, 0, gap, gap, -HALF_PI, -QUARTER_PI);
    pop();
    strokeWeight(mStroke);
    stroke(0);
    circle(aniWidthOn2, height - aniHeightOn3, gapOn2);
    rect(aniWidthOn2 - gapOn2, height - aniHeightOn3, gap, gapOn2);
    let lh = height - aniHeightOn3 + gap;
    line(aniWidthOn2, lh - gapOn2, aniWidthOn2, lh);
    if (timer > 0) {
        fill(255, 255, 0);
        stroke(255, 255, 0);
        line(aniWidthOn2, lh, gap, lh);
        line(gap, lh, gap, aniHeightOn3 + gap);
        stroke(0);
        line(aniWidthOn2, lh, width - aniWidthOn3 / 2, lh);
        line(width - aniWidthOn3 / 2, lh, width - aniWidthOn3 / 2, height - aniHeightOn3 / 2);
    } else {
        noFill();
        stroke(255, 255, 0);
        line(aniWidthOn2, lh, width - aniWidthOn3 / 2, lh);
    ...
    stroke(0);
    ellipse(gap, aniHeightOn3, gap, gap2);
    noFill();
    triangle(gap - gapOn4, aniHeightOn3, gap + gapOn4, aniHeightOn3, gap, aniHeightOn3 + gap);
    fill(100);
    rect(gapOn2, aniHeightOn3 + gap34, gap, gapOn2);
    fill(0, 0, 255);
    noStroke();
    rect(width - aniWidthOn3, aniHeightOn3 / 2, gap2, 2 * aniHeightOn3);
    let water = 1 - (cos(millis() / per) + 1) / 2;
    fill(255);
    rect(width - aniWidthOn3, aniHeightOn3 / 2, gap2, 2 * aniHeightOn3 * water);
    noFill();
    stroke(0);
    rect(width - aniWidthOn3, aniHeightOn3 / 2, gap2, 2 * aniHeightOn3);
}
...
function skipping() {
    ...
    circle(aniWidthOn2, aniHeightOn2, aniWidthOn2);
    noStroke();
    if (rCol[4].levels[0] == 255) {
        fill(0, 255, 255);
    } else {
        fill(color([198, 134, 66]));
    }
    circle(aniWidthOn2, aniHeightOn2, aniWidthOn4);
    fill(0);
    circle(aniWidthOn2, aniHeightOn2, aniWidthOn4 / 2);
    push();
    translate(aniWidthOn2, aniHeightOn2);
    rotate(-QUARTER_PI);
    if (timer > 0.8) {
        strokeWeight(mStroke);
        stroke(0, 255, 0);
    ...
    fill(200);
    noStroke();
    rect(aniWidthOn4 + gapOn4, -gapOn2, width, gap);
    pop();
}
...
function drawCar() {
    if (!("car" in aniLayers)) {
        aniLayers["car"] = createGraphics(width, height);
        aniLayers["car window"] = createGraphics(width, height);
        ...
        for (let i = 0; i < body.length; i++) {
            aniLayers["car"].curveVertex(body[i][0] * width, body[i][1] * height);
        }
        ...
        for (let i = 0; i < windows.length; i++) {
            aniLayers["car window"].curveVertex(windows[i][0] * width, windows[i][1] * height);
        }
        aniLayers["car window"].endShape();
        // aniLayers['car'].fill(255, 255, 0);
        // aniLayers['car'].ellipse(0.12*width, 0.77 * height, 0.04*width, 0.06*width);
        // aniLayers['car'].ellipse(0.9*width, 0.68 * height, 0.04*width, 0.06*width);
        // let lights = [[0.12, 0.77], [0.9, 0.7]];
        // let wheels = [[0.23, 0.9], [0.83, 0.9]];
        aniLayers["car"].fill(0);
        aniLayers["car"].circle(0.23 * width, 0.88 * height, 0.16 * width);
        aniLayers["car"].circle(0.83 * width, 0.88 * height, 0.16 * width);
        aniLayers["car"].fill(100);
        aniLayers["car"].circle(0.23 * width, 0.88 * height, 0.08 * width);
        aniLayers["car"].circle(0.83 * width, 0.88 * height, 0.08 * width);
    }
}
...
function harmtobenefit() {
    ...
    image(aniLayers["car"], 0, -aniHeightOn3 / 2);
    ...
    rect(ex, ey, gap, gapOn2);
    rect(ex - gap34, ey, gapOn6, gapOn2);
    rect(ex - gap34 / 2, ey, gap34, gapOn6);
    rect(ex, ey - gapOn3, gapOn2, gapOn6);
    rect(ex + gap34, ey, gapOn6, gapOn2);
    rect(ex + gap34 / 2, ey, gap34, gapOn6);
    c = lerpColor(color(200, 255, 255), color(255, 255, 100), timer);
    tint(c);
    image(aniLayers["car window"], 0, -aniHeightOn3 / 2);
    noTint();
    rectMode(CORNER);
}
...
function drawFBLoop() {
    if (!("fbl" in aniLayers)) {
        aniLayers["fbl"] = createGraphics(width, height);
        ...
        aniLayers["fbl"].line(0, aniHeightOn4, width, aniHeightOn4);
        ...
        let errorY = gapOn3;
        let outputSignalX = width - aw8 - gapOn3 / 3;
        ...
        aniLayers["fbl overlay"] = createGraphics(width, height);
...
function feedback() {
    ...
    let inputSignalY = gapOn3;
    fill(255);
    rect(inputSignalX, inputSignalY, gapOn3, gHeight);
    fill(255, 0, 0);
    rect(inputSignalX, inputSignalY + (1 - input) * gHeight, gapOn3, gHeight * input);
    let errorX = aniWidthOn3 - gapOn6;
    let errorY = gapOn3;
    fill(255);
    rect(errorX, errorY, gapOn3, gHeight);
    fill(255, 0, 0);
    rect(errorX, errorY + (1 - error / 2) * gHeight - gHeight / 2, gapOn3 / 2, (gHeight * error) / 2);
    fill(0, 255, 0);
    rect(errorX + gapOn3 / 2, errorY + (1 - error / 2) * gHeight - gHeight / 2, gapOn3 / 2, (gHeight * error) / 2);
    let outputSignalX = width - aw8 - gapOn3 / 3;
    let outputSignalY = gapOn3;
    fill(255);
    rect(outputSignalX, outputSignalY, gapOn3, gHeight);
    fill(0, 255, 0);
    rect(outputSignalX, outputSignalY + (1 - output) * gHeight, gapOn3, gHeight * output);
    let targetX = aniWidthOn2 + gapOn2;
    let targetY = aniHeightOn2 - gHeight / 2 + gapOn3; //aniHeightOn4 * 3 - gHeight + gapOn4 - mStroke/2;
    fill(255);
    rect(targetX, targetY, gapOn3, gHeight);
    fill(255, 0, 255);
    rect(targetX, targetY + (1 - target) * gHeight, gapOn3, gHeight * target);
    let cx = targetX + gap; // + gapOn4;
    image(aniLayers["fbl overlay"], 0, 0);
    strokeWeight(mStroke);
    stroke(0);
    push();
    translate(cx, targetY + gHeight / 2);
...
function intermediary() {
    ...
    noStroke();
    fill(rCol[0]);
    rect(0, wh, width, height);
    if (counter == 0) {
        fill(255);
        circle(aniWidthOn3 - gap34, wh - gap34, gap2);
    }
    fill(rCol[1]);
    rect(2 * aniWidthOn3, nh - gapOn2, gapOn2, gapOn2);
    fill(100);
    rect(aniWidthOn3 - gap34 - gapOn4, hh - gap - gapOn2, gapOn2, gap);
    rect(2 * aniWidthOn3, hh - gap2, gapOn2, gap);
    fill(rCol[2]);
    rect(aniWidthOn3 - gapOn2, hh - (gap + gapOn4 / 2), gap, gapOn4);
    rect(2 * aniWidthOn3 + gapOn2, hh - gap - gapOn2 - gapOn4 / 2, gap, gapOn4);
    image(aniLayers["nail small"], aniWidthOn3 - gap, nh);
    image(aniLayers["nail small"], 2 * aniWidthOn3, nh);
}
...
function selfservice() {
    ...
    stroke(0);
    strokeWeight(mStroke);
    rect(aniWidthOn2, aniHeightOn2, gap3 + gapOn3, gap2);
    line(aniWidthOn2, py, px + gap, py);
    line(gapOn2, ry, rx, ry);
    fill(0);
    triangle(px + gap, py, px + gap34, py - gapOn6, px + gap34, py + gapOn6);
    noStroke();
    // waste
    fill(rCol[1]);
    square(px, py + ramp * gap2, size);
    fill(255);
    circle(px, py + ramp * gap2, size);
    // part
    fill(rCol[1]);
    circle(px + ramp * gap3, py, size);
    square(rx, ry - ramp * dry, size);
    square(rx + drx * ramp, py, size);
    // waste
    fill(rCol[1]);
    square(rx + iramp * drx, ry, size);
    let subSquareSize = 0.85 * size; // sqrt ( Pi (d/2)^2) = sqrt(pi) * (d/2)
    square(rx - iramp * drx, ry, subSquareSize);
    fill(255);
    circle(rx + iramp * drx, ry, size);
    // processors
    fill(rCol[0]);
    square(rx, ry, size * 1.5);
    square(aniWidthOn2, py, size * 1.5);
    fill(rCol[2]);
...
function copying() {
    ...
    if (ramp == 0) {
        let cv = t * 100;
        fill(255, 255 - cv, 255 - cv);
        ellipse(aniWidthOn4, aniHeightOn2, aniWidthOn2, height);
        // rect(0, 0, aniWidthOn2, height);
        alarm = sin(millis() / (per / 2));
    }
    let tx = aniWidthOn4 - gap + gap * ramp;
    let tx2 = tx + aniWidthOn2;
    let ty = aniHeightOn3;
    image(aniLayers["trophy"], tx, ty);
    image(aniLayers["trophy"], tx2, ty);
    fill(100);
    let bsx = tx2 + gap34;
    rect(tx2 + gap34, ty, gap34, gap, gap, gapOn4, gapOn3, gapOn6);
    bsx += gap34;
    triangle(bsx, ty - gapOn6, bsx - gapOn6, ty + gapOn3, bsx + gapOn6, ty);
    let pw = gap;
    let ph = gap;
    ...
    fill(rCol[1]);
    rect(px, py - alarm * gapOn2, gap, gapOn4);
    rect(px + gapOn4, py - alarm * gapOn2, gapOn2, gap);
    rect(px2, py, gap, gapOn4);
    fill(rCol[0]);
    rect(px, py + gapOn4, gap, gap);
    rect(px2, py + gapOn4, gap, gap);
    stroke(0);
    strokeWeight(mStroke);
    let sx = px + gapOn4;
    let ex = px + gap34;
...
function cheapshort() {
    ...
    if (tri2 == 3) {
        ...
        if (tri3 != 3 && tri3 != 4) {
            rect(gapOn2, aniHeightOn2 + offsets[1] - gapOn2, gapOn4, gap);
        }
        fill(rCol[0]);
    ...
    if (tri2 == 0) {
        ...
        if (tri3 != 3 && tri3 != 4) {
            rect(gapOn2, aniHeightOn2 + offsets[1] - gapOn2, gapOn4, gap);
            rect(gapOn2, aniHeightOn2 + offsets[2] - gapOn2, gapOn4, gap);
        }
        fill(rCol[0]);
        cy = aniHeightOn2 - gapOn2 + (1 - tri) * offsets[1] + gapOn2;
        circle(cx, cy, gapOn2);
    ...
    // wall
    fill(rCol[1]);
    rect(width - gap34, 0, gapOn2, height);
    // paddle
    let px = aniHeightOn2 - gapOn2 + (1 - tri) * offsets[tri2];
    rect(gapOn4, px, gapOn2, gap);
}
...
function mechanicalsubstitution() {
    ...
    //brushed()
    push();
    translate(aniWidthOn4 - gapOn6, aniHeightOn2);
    fill(c1);
    rect(-gap, 0, gapOn4, gap3);
    fill(c2);
    rect(gap, 0, gapOn4, gap3);
    rotate(currentAngle);
    ...
    fill(c1);
    let xoff = gap34 / 2;
    rect(-xoff, 0, gap34, gapOn2);
    fill(c2);
    rect(xoff, 0, gap34, gapOn2);
    arc(0, 0, gap34, gap34, PI, 0);
    fill(c1);
    arc(0, 0, gap34, gap34, 0, PI);
    fill(255);
    rect(0, 0, gap34, gapOn6 / 2);
    pop();
    strokeWeight(mStroke);
    stroke(0);
    let yend = gapOn2;
    ...
    line(x1, aniHeightOn2, x1, yend);
    line(x2, aniHeightOn2, x2, yend);
    noStroke();
    fill(255, 230, 0);
    circle(x1, aniHeightOn2, gapOn4);
    circle(x2, aniHeightOn2, gapOn4);
    fill(255, 0, 0);
    circle(x1, yend, gapOn2);
    fill(0, 0, 255);
    circle(x2, yend, gapOn2);
    //brushless
    let xc = width - aniWidthOn4 + gapOn6;
    ...
    x2 = xc;
    let x3 = xc + xoff;
    line(x1, gapOn2, x1, xy + 0.35 * aniHeightOn4);
    line(x2, gapOn2, x2, xy - aniHeightOn4);
    line(x3, gapOn2, x3, xy + 0.35 * aniHeightOn4);
    noStroke();
    redC = color(255, 0, 0);
    blueC = color(0, 0, 255);
    off = color(100);
    ...
    fill(stators[phase][0][0]);
    circle(x2, gapOn2, gapOn2);
    fill(stators[phase][1][0]);
    circle(x3, gapOn2, gapOn2);
    fill(stators[phase][2][0]);
    circle(x1, gapOn2, gapOn2);
    let rotations = [0, TWO_PI / 3, (2 * TWO_PI) / 3];
    for (let i = 0; i < 3; i++) {
        push();
        translate(xc, xy);
        rotate(rotations[i]);
        fill(stators[phase][i][0]);
        rect(0, -aniHeightOn4, gapOn2, gapOn4);
        fill(stators[phase][i][1]);
        rect(0, -aniHeightOn4 - gapOn4, gapOn2, gapOn4);
        pop();
    }
    push();
    translate(xc, xy);
    ...
    fill(redC);
    let arad = gap + gapOn2;
    arc(0, 0, arad, arad, PI, 0);
    fill(blueC);
    arc(0, 0, arad, arad, 0, PI);
    fill(255);
    circle(0, 0, gap34);
    pop();
    stroke(200);
...
function pneumaticsAndHydraulics() {
    ...
    let time = millis();
    fill(0, 0, 255);
    rect(aniWidthOn2, aniHeightOn4, aniWidthOn2, 3 * aniHeightOn4);
    rect(0, 3 * aniHeightOn4, aniWidthOn2, aniHeightOn4);

    fill(100, 100, 100);
    beginShape();
    vertex(aniWidthOn2 - gap, height);
    vertex(aniWidthOn2 - gapOn2, 0);
    vertex(aniWidthOn2 + gapOn2, 0);
    vertex(aniWidthOn2 + gap, height);
    endShape();
    fill(0, 0, 255);
    rect(aniWidthOn2 - gap, aniHeightOn2 + gapOn6, gap2, gapOn3, gapOn6);
    rect(aniWidthOn2 - gap, aniHeightOn2 + gapOn6, gapOn3, aniHeightOn2, gapOn6);
    for (let i = 0; i < 50; i++) {
        fill(0, 200, 200);
        offy = random(gapOn2);
        ...
        circle(aniWidthOn2 - 5 * gapOn6 + offx, aniHeightOn2 + 3 * gapOn4 + offy, rad);
    }
    fill(255, 0, 0);
    push();
    translate(aniWidthOn2, aniHeightOn2);
...
function shellsAndFilms() {
    ...
    noStroke();
    fill(100, 100, 100);
    rect(0, aniHeightOn4 + ch - gapOn6/2, width, gap2);
    fill(101, 67, 33);
    rect(0, 0, width, gap);
    fill(50, 50, 50);
    circle(gap, gap, gap);
    rect(gapOn2, gap, gapOn6, (height - gap2 - gapOn6/2) * abs(sincurrentAngle));
    fill(200, 200, 200);
    circle(gap, gap, gapOn2);
    image(aniLayers["car"], aniWidthOn2 - gap2, aniHeightOn4, cw, ch);
    image(aniLayers["car window"], aniWidthOn2 - gap2, aniHeightOn4, cw, ch);
    fill(101, 67, 33);
    rect(aniWidthOn2 + gap2, aniHeightOn2-gap, gap, gap2);
    fill(255, 215, 0);
    circle(aniWidthOn2 + gap2 + gapOn6, aniHeightOn2, gapOn6)
...
function eggPan() {
    ...
    strokeWeight(gapOn6);
    fill(100, 100, 100);
    translate(aniWidthOn3 + sincurrentAngle * gapOn4, 0);
    circle(0, aniHeightOn2, aniWidthOn3);
    noStroke();
    fill(200, 200, 200);
    rect(aniWidthOn3 * 0.5, aniHeightOn2 - gapOn4 * 0.4, gap, gapOn4);
    fill(100, 100, 100);
    rect(aniWidthOn3 * 0.5 + gap, aniHeightOn2 - gapOn4, gap, gapOn2);
    pop();
    noStroke();
    fill(255);
    circle(aniWidthOn3, aniHeightOn2, gap);
    fill(255, 200, 0);
    circle(aniWidthOn3, aniHeightOn2, gapOn2);
}
