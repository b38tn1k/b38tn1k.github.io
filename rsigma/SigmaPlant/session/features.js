...
class Feature {
    ...
    drawButtonsAndLabels(zoom, cnv, myStrokeColor = getColor("outline"), myFillColor = getColor("secondary")) {
        if (this.notYetDrawnLabelAndButtons) {
            ...
            if (this.mode != 'delete') { // Draw buttons
                for (let button of this.buttons) { // Convert button position to screen coordinates
...
class Process extends Feature {
    ...
    collectBuses() {
        let inputIndices = this.plant.features
            .map((feature, index) => feature.type == 'source' ? index : null) .filter(index => index !== null);
            
            this.buses['source'] = new Set(inputIndices);

            let outputIndices = this.plant.features.map((feature, index) => feature.type == 'sink' ? index : null).filter(index => index !== null);
        ...
        setupIOButtons(buttonSize = BUTTON_SIZE) {
            ...
            this.createIOButtons('source', FeatureUIInputButton, 'Input', 'i_connect', xIncrementSource, buttonSize);
            this.createIOButtons('sink', FeatureUIOutputButton, 'Output', 'o_connect', xIncrementSink, buttonSize);
        }

        removeInvalidButtons(validIDs) {
            for (let button of this.buttons) {
                if ((button instanceof FeatureUIInputButton || button instanceof FeatureUIOutputButton) && !validIDs.includes(button.targetID)) {
                    button.mode = 'delete';
                }
            }
        }
    ...
    class Source extends Feature {
        ...
        initButtons(buttonSize) {
            this.buttons.push(new FeatureUIButtonMove('Move', 0, 0, buttonSize, () => this.setMode('move')));
            this.buttons.push(new FeatureUIButtonClose('Xdelete', 1, 0, buttonSize, () => this.startDelete()));
            this.buttons.push(new FeatureUIOutputButton('Output', 0.5, 1, buttonSize, () => this.setMode('o_connect')));
        }
    ...
    class ParentLink extends Feature {
        ...
        initDataLabels(buttonSize) {
            this.dataLabels['title'] = new FeatureDataTextLabelTrigger(0.5, 0.3, 'PARENT', buttonSize, () => this.transitionPlant()); // bad, fix with a mode
        }
    ...
    class Sink extends Feature {
        ...
        initButtons(buttonSize) {
            this.buttons.push(new FeatureUIButtonMove('Move', 0, 0, buttonSize, () => this.setMode('move')));
            this.buttons.push(new FeatureUIButtonClose('Xdelete', 1, 0, buttonSize, () => this.startDelete()));
            this.buttons.push(new FeatureUIInputButton('Input', 0.5, 0, buttonSize, () => this.setMode('i_connect')));
        }
...
class Metric extends Feature {
    ...
    class Split extends Feature {
        ...
        initButtons(buttonSize) {
            ...
            this.buttons.push(new FeatureUIInputButton('Input', 0.5, 0, buttonSize, () => this.setMode('i_connect')));
            this.buttons.push(new FeatureUIOutputButton('Output', 0.5, 1, buttonSize, () => this.setMode('o_connect')));
            this.buttons.push(new FeatureUIOutputButton('Output', 1, 1, buttonSize, () => this.setMode('o_connect')));
        }
    ...
    class Merge extends Feature {
        ...
        initButtons(buttonSize) {
            this.buttons.push(new FeatureUIButtonMove('Move', 0, 0, buttonSize, () => this.setMode('move')));
            this.buttons.push(new FeatureUIButtonClose('Xdelete', 1, 0, buttonSize, () => this.startDelete()));
            this.buttons.push(new FeatureUIInputButton('Input', 0.3, 0, buttonSize, () => this.setMode('i_connect')));
            this.buttons.push(new FeatureUIInputButton('Input', 0.6, 0, buttonSize, () => this.setMode('i_connect')));
            this.buttons.push(new FeatureUIOutputButton('Output', 0.5, 1, buttonSize, () => this.setMode('o_connect')));
        }
