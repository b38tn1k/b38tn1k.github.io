...
function draw() {
  ...
  plusTrackButton = drawButton(gr, '+ zone', 4, gr.height - 24, 40, 20, border, border);
  deleteButton = drawButton(gr, 'delete', 50, gr.height - 24, 40, 20, border, border, deleteModeOn, colors[23]);
  clearButton = drawButton(gr, 'clear all', 96, gr.height - 24, 50, 20, border, border);
  demoButton = drawButton(gr, 'demo', 152, gr.height - 24, 40, 20, border, border);
  screenShotButton = drawButton(gr, 'screenshot', 198, gr.height - 24, 70, 20, border, border);
  saveButton = drawButton(gr, 'save', 274, gr.height - 24, 40, 20, border, border);
  loadButton = drawButton(gr, 'load', 320, gr.height - 24, 40, 20, border, border);
  helpButton = drawButton(gr, 'help', 366, gr.height - 24, 40, 20, border, border);
  let temp = llHead;
  while (!(temp === null)){
    if (!(temp.low === null)) {
      if ((temp.pixelY + 2*temp.pixelH) < gr.height){
  ...
  if (screenshot) {
    gr.noStroke();
    gr.fill(colors[4]);
    gr.rect(0, gr.height - 28, gr.width, 30);
  }
  image(gr, border, border);
  if (screenshot) {
