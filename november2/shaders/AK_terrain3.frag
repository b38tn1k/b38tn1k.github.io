...
vec4 renderTerrain(vec2 uv) {
  ...
  float upFacing = clamp(normal.y * 0.8 + 0.2, 0.0, 1.0); // surfaces facing "up"
  ...
  rockColor = mix(rockColor, mix(rockColor, coolDeep, 0.55), depthFactor * 0.45);
  ...
  // Underwater caustic streaks, shared with water look so things feel unified
  vec2 causticUv = uv * (uResolution / 1500.0);
  float causticField = fworleyWater(causticUv);
  float waveField = sea_octaveWater((uv - vec2(0.5, 0.7)) * vec2(5.0, 3.0),
                                    0.4, uTime * 0.6);
  float causticCombined = causticField * (0.5 + 0.5 * clamp(waveField, 0.0, 2.0));
  float causticMask = smoothstep(0.78, 0.98, causticCombined);

  vec3 causticColor = vec3(1.0, 0.96, 0.78);
  // Stronger on exposed convex areas (low AO, upward-facing)
  float causticStrength = (1.0 - ao) * (0.4 + 0.6 * upFacing);
  rockColor += causticColor * causticMask * causticStrength * 0.55;
